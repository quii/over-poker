...
class HandTest extends FunSpec {
  ...
  it("should not find anything interesting") {
    ...
    Hand.getValues(hand, flop) should contain(HighCard(Jack))
  }
  ...
  it("finds the players best hand") {
    val flopWithThreeOfAKind = Flop(3 of Spades, 3 of Hearts, 3 of Diamonds)
    val playerHand = PlayerHand(5 of Spades, Ace of Spades)
    Hand.playersBestHand(playerHand, flopWithThreeOfAKind) should be(HighCard(Ace))
  }

  it("finds players best hand when the player has two hands") {
    val flopWithThreeOfAKind = Flop(3 of Spades, 3 of Diamonds, 3 of Clubs)
    val playerHandMakesFullHouseAndPair = PlayerHand(2 of Spades, 2 of Diamonds)
    Hand.playersBestHand(playerHandMakesFullHouseAndPair, flopWithThreeOfAKind) should be(FullHouse(3,2))
  }
}
