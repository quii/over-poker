...
object PossibleHands {
  ...
  private def afterFlop(communityCards: Vector[Card]): Boolean = communityCards.length >= 3
  private def beforeTurn(communityCards: Vector[Card]): Boolean = communityCards.length < 4
  private def afterTurn(communityCards: Vector[Card]): Boolean = communityCards.length >= 4
  private def afterRiver(communityCards: Vector[Card]): Boolean = communityCards.length >= 5

  private def conformsToBeforeFlopRules(playerHand: PlayerHand, hand: Set[Card]): Boolean = {
    hand.contains(playerHand.card1) || hand.contains(playerHand.card2)
  }

  private def conformsToAfterFlopBeforeTurnRules(playerHand: PlayerHand, communityCards: Vector[Card], hand: Set[Card]): Boolean = {
  ...
  private def conformsToAfterTurnRules(playerHand: PlayerHand, communityCards: Vector[Card], hand: Set[Card]): Boolean = {
